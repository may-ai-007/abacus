...
class SoundManager {
    ...
    loadSounds() {
        ...
        // 确保音效已加载，如果5秒后还没加载完成，则创建默认音效
        setTimeout(() => {
            if (!this.soundBuffers.upperBead) {
                console.warn('Upper bead sound not loaded, creating default');
                this.createDefaultSound('upperBead');
            }
            if (!this.soundBuffers.lowerBead) {
                console.warn('Lower bead sound not loaded, creating default');
